...
$connect = mysql_connect("localhost", "actoze", "0453acac") or die("DB Connect Error!!");

mysql_query("SET NAMES 'utf8'");

$db = mysql_select_db("actoze", $connect) or die("DB Select Error!!");

 
if ($_REQUEST['select'] == "list") {

	/* we query through our database to search for a username that has been entered */
	$result2 = mysql_query("select bo_subject from g5_board where bo_table LIKE '".$_REQUEST['month']."w%';");
// echo "select bo_subject from g5_board where bo_table LIKE '".$_REQUEST['month']."w%';";
	while ($array2 = mysql_fetch_array($result2)) {
		$results[] = array('subject' => $array2['bo_subject']);
	}
