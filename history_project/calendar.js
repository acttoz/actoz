...
var weekList = new Array("3w2", "3w3", "3w4", "3w5", "4w2", "4w3", "4w4", "5w2", "5w3", "5w4", "5w5", "6w1", "6w2", "6w3", "6w4", "7w2", "7w3", "7w4", "9w1", "9w2", "9w3", "9w4", "10w2", "10w3", "10w4", "10w5", "11w2", "11w3", "11w4", "11w5", "12w1", "12w2", "12w3", "12w4", "2w1", "2w2", "2w3", "2w4");
...
function showCalendar(day, month, year) {
	...
	var calStr = "<table border=0 cellpadding=5 cellspacing=1 align=center bgcolor=#CCCCCC style='min-height: 500px;'>"

	calStr += "<tr bgcolor=white><td colspan=9>"

	calStr += "<table border=0 cellpadding=0 cellspacing=0 align=center width=100% style='table-layout:fixed'>"
	...
	for (var i = 0; i < monthDays[month]; i++) {
		...
		if ((i + firstDay + 1) % 7 == 1){
			calStr += "width='248'"}else{calStr += "width='80'"}

		if (dayCount == nowd) {
			calStr += " align=center bgcolor='#DFE7DE'><font size='3'><b>" // 오늘 날짜일때 배경색 지정,글자 진하게

		} else {

			calStr += " align=center><font size='3'>" // 오늘 날짜가 아닐때 배경색 지정
		}
		if (weekList.indexOf(week) > 0) {
			if ((i + firstDay + 1) % 7 == 0) {
				calStr += "<a href='http://actoze.dothome.co.kr/bbs/board.php?bo_table=" + review + "'>"// 링크설정
				calStr += dayCount++// 날짜
				calStr += "<br/>(토&nbsp;&nbsp;의)"// 날짜
				...
				calStr += "<a href='http://actoze.dothome.co.kr/bbs/board.php?bo_table=" + week + "'>"// 링크설정
				calStr += dayCount++// 날짜
				calStr += "<br/>(글쓰기)"// 날짜
			}
